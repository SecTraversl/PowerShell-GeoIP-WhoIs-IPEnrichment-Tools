...
.NOTES
  Name: Get-GeoIP.ps1
  Author: Travis Logue
  Version History: 4.1 | 2022-01-01 | Added "-UseBasicParsing", see Notes
  Dependencies:
  Notes:
  ...
  - 2022-01-01 - # Received an error with a newly added user account when using this command line...
      ...
      # The fix is to use the "-UseBasicParsing" Switch Parameter for Invoke-WebRequest, according to this:  https://stackoverflow.com/questions/38005341/the-response-content-cannot-be-parsed-because-the-internet-explorer-engine-is-no
...
function Get-GeoIP {
  ...
  [Alias('GeoIP')]
  param (
    [Parameter(HelpMessage='Reference the IP Address to look up.', Mandatory)]
    [string[]]
    $IPAddress
  )
  ...
  process {
    ...
    foreach ($IP in $IPAddress) {
      ...
      # Received an error with a newly added user account when using this command line...
      # $LookupResults = Invoke-WebRequest -Uri "https://ipinfo.io/$IP/json" | ConvertFrom-Json
      ...
      # The fix is to use the "-UseBasicParsing" Switch Parameter for Invoke-WebRequest, according to this:  https://stackoverflow.com/questions/38005341/the-response-content-cannot-be-parsed-because-the-internet-explorer-engine-is-no
      $LookupResults = Invoke-WebRequest -Uri "https://ipinfo.io/$IP/json" -UseBasicParsing | ConvertFrom-Json
